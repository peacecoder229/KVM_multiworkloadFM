...
#for workloads in "nginx"; do
  ...
  for cache in "${cache_list[@]}"; do
    ...
    tmc -c "./run_testcases.sh $result_dir $config2" -n -x mchowdh1 -e /opt/intel/sep/config/edp/filtered_events.txt -u -S 5 -a "l2cat" -i "$case_name" -w thread,socket,core -D $case_name
