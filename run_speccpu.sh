...
if [[ $VM_EXP == True ]]; then
  ...
  cp /root/speccpu_script/workload_cpat.sh ./
  cp /root/speccpu_script/shrc ./
  start_core=0
  end_core=$(($(getconf _NPROCESSORS_ONLN)-1))
  echo "run_speccpu: ./workload.sh $workload $start_core $end_core $n_iteration $asm"
  #./workload.sh $workload $start_core $end_core $n_iteration $asm
  ./workload_cpat.sh $workload $start_core $end_core $n_iteration $asm
else
...
#fi
log_file=$(grep "The log for this run is in" workload.log | cut -d" " -f8)
elapsed_times=$(grep -Irn "Total elapsed time:" $log_file | cut -d" " -f17)
echo "$elapsed_times" > /root/elapsed_times.txt
avg_elapsed_time=$(cat /root/elapsed_times.txt | awk '{if($1 != 0) {total += $1}} END {print total/NR}')
