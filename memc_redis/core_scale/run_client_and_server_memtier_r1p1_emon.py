...
def get_opts():
    ...
    parser.add_option("--proto",  dest="protocol",
                      help="provide starting port of the server core group", default=9001)
...
if __name__ == "__main__": 
    ...
    emondir = "Twitter"
    emonname = options.mark_name
    emoncmd = "ssh $SUT \"/pnpdata/emon/run_emon_sriov.sh 10 " + emonname + " " + emondir + " \""
    pqoscmd = "ssh -f $SUT \"/usr/local/bin/pqos -r -i 2 -m \"all:[2-31]\" -o /pnpdata/emon/results/pqos_" + options.connection + " \""
    ...
    #os.system("ssh $SUT \"source /opt/intel/sep_vars.sh ; emon -stop\"")
    #os.system("ssh SUT \"pkill -9 pqos\"")
