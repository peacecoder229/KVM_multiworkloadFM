...
def get_opts():
    ...
    parser.add_option("--pclient",  dest="primeclient",
                      help="save report name", default=None)

    parser.add_option("--sport",  dest="startport",
                      help="save report name", default=9001)
...
def run_memtier(options):
    ...
    for port in range(int(options.startport), int(options.startport)+corenum):
        core = clientcores.pop()
