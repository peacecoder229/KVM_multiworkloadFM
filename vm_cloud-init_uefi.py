...
''' % (iso_name, iso_name,rsa_key))
    ...
    if ("qat" in iso_name):
        ...
        #image_name = 'spec-golden-image.qcow2'
        #image_name = 'centos_generic_golden_image.qcow2'
        image_name = 'mlc_golden.qcow2'
        print(f"Copying  {path_prefix}/vmimages/{image_name} {path_prefix}/{vm_storage}/{iso_name}.qcow2")
        os.system("cp %s/vmimages/%s %s/%s/%s.qcow2" %
                  (path_prefix, image_name, path_prefix, vm_storage,iso_name))
        
        #os.system("cp ./vmimages/cpu_inference_golden_vmimage.qcow2 %s/%s/%s.qcow2" %
        #          (path_prefix, vm_storage,iso_name))
    elif ("5g" in iso_name):
        print("using CPU golden image ")
...
def generate_commands(assign_random=False):
    ...
    for port_no in sorted(Tile_Map.keys()):
        ...
        for tile in sorted(tiles.keys()):
            # If a workload has atleast 1 VM, go inside the loop create that many VMs
            for count in range(0, tiles.get(tile)):
                ...
                if (tile == "QAT"):
                    ...
                    # Pin VMs to the specified physical cpus
                    ...
                    # muktadir: The options is needed for the new images, but when I add it it, VMs don't boot up, they are stuck in the boot menu
                    CMD_FORMAT = "virt-install --import -n %s-%02d -r %s --vcpus=%s --os-type=linux --os-variant=centos7.0 --accelerate --disk path=%s/%s/%s.qcow2,format=raw,bus=virtio,cache=writeback --disk path=%s/%s/%s.iso,device=cdrom %s %s --noautoconsole --cpu host-passthrough,cache.mode=passthrough --nographics --boot uefi --sysinfo system.serial='ds=nocloud'"
                    ...
                    #CMD_FORMAT_REMOVE_CD = "virt-install --import -n %s-%02d -r %s --vcpus=%s --os-type=linux --os-variant=centos7.0 --accelerate --disk path=%s/%s/%s.qcow2,format=raw,bus=virtio,cache=writeback %s --noautoconsole --cpu host-passthrough,cache.mode=passthrough"
                    # muktadir: Ditto
                    CMD_FORMAT_REMOVE_CD = "virt-install --import -n %s-%02d -r %s --vcpus=%s --os-type=linux --os-variant=centos7.0 --accelerate --disk path=%s/%s/%s.qcow2,format=raw,bus=virtio,cache=writeback %s --noautoconsole --cpu host-passthrough,cache.mode=passthrough --boot uefi --sysinfo system.serial='ds=nocloud'"
...
def run_test(test_commands, test_commands_remove_cd):
    ...
    file1.write("virsh list --all --name|xargs -i virsh destroy {} ;  virsh list --all --name|xargs -i virsh undefine {} --nvram; sleep 5 \n ")
