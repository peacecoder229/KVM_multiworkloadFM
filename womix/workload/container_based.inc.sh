...
function stressapp()
{
	...
	docker run --rm --privileged --cpuset-cpus=$cpuset --name=$name -e MTHREAD=$ins  -v `pwd`/$result_path:/workdir  stressapp:latest
}
...
function speccpu2006()
{
	...
	docker run --rm --privileged --cpuset-cpus=$cpu_set --name=$name  -v `pwd`/$result_path:/SPECcpu/result  -e INSTANCES=$ins -e WORKLOAD=$comp speccpu2006:latest 
...
function speccpu2017()
{
	...
	docker run --privileged --rm --cpuset-cpus=$cpu_set --name=$name  -v `pwd`/$result_path:/SPECcpu/result -e INSTANCES=$ins -e WORKLOAD=$comp speccpu2017:latest
}
...
# 	docker run --name $name --cpuset-cpus=$cpu_set -v <data_path>:/root/mp4  ffmepg_eric:v1 /root/mp4/run_nginx.sh > $result_path/ffmpeg.log
# }

function mlc_container()
{
	cpu_set=$1
	result_path=$2
	ins=$3
...
#   ENV BUFFER=100m
	...
	docker run --rm --name=$name -e CPUSET=$cpu_set -e OPERATION=$comp mlc_operation:latest > $result_path/mlc_out.txt
}
...
function stressapp()
{
	...
	docker run --rm --name=$name -e CPUSET=$cpu_set -v `pwd`/$result_path:/workdir -e MTHREAD=$ins -e RUNTIME=$runtime stressapp 
}
